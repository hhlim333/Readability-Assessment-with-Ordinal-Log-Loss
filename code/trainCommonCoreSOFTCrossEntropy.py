...
def save(args, model, info, k_cnt, l_cnt,dfs):
    # save model
    os.makedirs('checkpointCommonCoreSOFTCrossEntropy', exist_ok=True)
    save_dir = os.path.join('checkpointCommonCoreSOFTCrossEntropy', f'{args.corpus_name}.{args.model.lower()}.{k_cnt}.{l_cnt}')
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch_item in enumerate(train_loader):
            ...
            for i, sample_out in enumerate(outputs):
                loss2 = loss2 + sample_out.view(1, 5).mm(softlabel[i].view(5, 1))[0][0]
