...
def save(args, model, info, k_cnt, l_cnt,dfs):
    # save model
    os.makedirs('checkpointCMTLBL', exist_ok=True)
    save_dir = os.path.join('checkpointCMTLBL', f'{args.corpus_name}.{args.model.lower()}.{k_cnt}.{l_cnt}')
...
for k_cnt in range(num_kfold):
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch_item in enumerate(train_loader):
            ...
            #0.0079, 0.0153, 0.03, 0.0402, 0.0372, 0.0519, 0.158, 0.0796, 0.1146, 0.137, 0.1603, 0.168
            loss = torch.tensor([0.0079 ** p, 0.0153 ** p, 0.03 ** p, 0.0402 ** p, 0.0372 ** p, 0.0519 ** p, 0.158 ** p, 0.0796 ** p, 0.1146 ** p, 0.137 ** p, 0.1603 ** p, 0.168 ** p]).to(args.device) * loss
