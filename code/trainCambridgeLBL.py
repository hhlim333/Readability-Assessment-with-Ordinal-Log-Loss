...
def save(args, model, info, k_cnt, l_cnt,dfs):
    # save model
    os.makedirs('checkpointCambridgeLBL', exist_ok=True)
    save_dir = os.path.join('checkpointCambridgeLBL', f'{args.corpus_name}.{args.model.lower()}.{k_cnt}.{l_cnt}')
...
for k_cnt in range(num_kfold):
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch_item in enumerate(train_loader):
            ...
            loss = torch.tensor([0.0574 ** p, 0.112 ** p, 0.2414 ** p, 0.2998 ** p, 0.2894 ** p]).to(args.device) * loss
