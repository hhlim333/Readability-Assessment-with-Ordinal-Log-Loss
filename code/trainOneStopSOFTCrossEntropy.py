...
def save(args, model, info, k_cnt, l_cnt,dfs):
    # save model
    os.makedirs('checkpointOneStopSOFTCrossEntropy', exist_ok=True)
    save_dir = os.path.join('checkpointOneStopSOFTCrossEntropy', f'{args.corpus_name}.{args.model.lower()}.{k_cnt}.{l_cnt}')
...
for k_cnt in range(num_kfold):
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch_item in enumerate(train_loader):
            ...
            softLabel = 1.2
            '''
            distances = [[0, softLabel, 1e32, 1e32, 1e32],
                        [softLabel, 0, softLabel, 1e32, 1e32],
                        [1e32, softLabel, 0, softLabel, 1e32],
                        [1e32, 1e32, softLabel, 0, softLabel],
                        [1e32, 1e32, 1e32, softLabel, 0]]
            '''
            distances = [[0, softLabel, 1e32],
                        [softLabel, 0, softLabel],
                        [1e32, softLabel, 0]]
            ...
            for i, sample_out in enumerate(outputs):
                loss2 = loss2 + sample_out.view(1, 3).mm(softlabel[i].view(3, 1))[0][0]
