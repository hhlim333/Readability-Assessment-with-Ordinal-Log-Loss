...
def save(args, model, info, k_cnt, l_cnt,dfs):
    # save model
    os.makedirs('checkpointCommonCoreLBL', exist_ok=True)
    save_dir = os.path.join('checkpointCommonCoreLBL', f'{args.corpus_name}.{args.model.lower()}.{k_cnt}.{l_cnt}')
...
for k_cnt in range(num_kfold):
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch_item in enumerate(train_loader):
            ...
            loss = torch.tensor([0.1223 ** p, 0.1221 ** p, 0.2015 ** p, 0.2521 ** p, 0.302 ** p]).to(args.device) * loss
