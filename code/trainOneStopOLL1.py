...
def save(args, model, info, k_cnt, l_cnt,dfs):
    # save model
    os.makedirs('checkpointOneStopOLL1', exist_ok=True)
    save_dir = os.path.join('checkpointOneStopOLL1', f'{args.corpus_name}.{args.model.lower()}.{k_cnt}.{l_cnt}')
...
for k_cnt in range(num_kfold):
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch_item in enumerate(train_loader):
            ...
            true_labels = [3*[labels[k].item()] for k in range(len(labels))]
            label_ids = len(labels)*[[k for k in range(3)]]
            distances = [[float(dist_matrix[true_labels[j][i]][label_ids[j][i]]) for i in range(3)] for j in range(len(labels))]
