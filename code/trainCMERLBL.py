...
def save(args, model, info, k_cnt, l_cnt,dfs):
    # save model
    os.makedirs('checkpointCMERLBL', exist_ok=True)
    save_dir = os.path.join('checkpointCMERLBL', f'{args.corpus_name}.{args.model.lower()}.{k_cnt}.{l_cnt}')
...
for k_cnt in range(num_kfold):
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch_item in enumerate(train_loader):
            ...
            num_classes = 12
            target_one_hot = F.one_hot(labels, num_classes)
            log_preds = F.log_softmax(outputs, dim=-1)
            loss = -target_one_hot * log_preds
            p=0.6
            #loss = torch.tensor([0.0529 ** p, 0.1057 ** p, 0.2425 ** p, 0.2994 ** p, 0.2996 ** p]).to(args.device) * loss
            #0.0131, 0.0252, 0.0333, 0.0533, 0.0519, 0.0499, 0.1189, 0.13, 0.1011, 0.1591, 0.1344, 0.1299
            loss = torch.tensor([0.0131 ** p, 0.0252 ** p, 0.0333 ** p, 0.0533 ** p, 0.0519 ** p, 0.0499 ** p, 0.1189 ** p, 0.13 ** p, 0.1011 ** p, 0.1591 ** p, 0.1344 ** p, 0.1299 ** p]).to(args.device) * loss
